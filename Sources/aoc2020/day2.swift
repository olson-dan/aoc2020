...
extension String {
    func partition(at: String.Element) -> (left: String, delim: String, right: String) {
        let s = self.split(separator: at, maxSplits: 1)
        if s.count == 2 {
            return (String(s[0]), String(at), String(s[1]))
        } else {
            return (String(s[0]), "", "")
        }
    }
    func countCharacter(_ c: String.Element) -> Int {
        var count = 0;
        for ch in self {
            if ch == c {
                count += 1
...
func is_valid_password(_ s: String) -> (one: Bool, two: Bool) {
    let p = s.partition(at: ":")
    let rules = p.left.partition(at: " ")
    let letter = rules.right
    let counts = rules.left.partition(at: "-")
    let min = Int(counts.left) ?? 0
    let max = Int(counts.right) ?? 0
    let password = p.right
    let count = password.countCharacter(Character(letter))
    let x = String(password[password.index(password.startIndex, offsetBy: min)])
    let y = String(password[password.index(password.startIndex, offsetBy: max)])
    let one = count >= min && count <= max
    let two = x != y && (x == letter || y == letter)
    return (one, two)
}

func day2_part1() {
    var count = 0
    for line in day2_input.split(separator: "\n") {
        if is_valid_password(String(line)).one {
            count += 1
        }
    }
    print("\(count)")
...
func day2_part2() {
    var count = 0
    for line in day2_input.split(separator: "\n") {
        if is_valid_password(String(line)).two {
            count += 1
        }
    }
    print("\(count)")
