...
for p in passports:
    ...
    for x in p:
        ...
        d[k] = v
    ps.append(d)
    if all([x in d for x in required]):
        part1_valid += 1
        part2 = True
        byr = int(d['byr'])
        part2 = part2 and byr >= 1920 and byr <= 2002
        iyr = int(d['iyr'])
        part2 = part2 and iyr >= 2010 and iyr <= 2020
        eyr = int(d['eyr'])
        part2 = part2 and eyr >= 2020 and eyr <= 2030
        if d['hgt'].endswith('cm'):
            hgt = int(d['hgt'].replace('cm',''))
            part2 = part2 and hgt >= 150 and hgt <= 193
        elif d['hgt'].endswith('in'):
            hgt = int(d['hgt'].replace('in',''))
            part2 = part2 and hgt >= 59 and hgt <= 76
        else:
            part2 = False
        if d['hcl'].startswith('#'):
            def valid(x):
                return (x >= '0' and x <= '9') or (x >= 'a' and x <= 'f')
            part2 = part2 and len(d['hcl']) == 7 and all([valid(x) for x in d['hcl'][1:]]) 
        else:
            part2 = False
        part2 = part2 and d['ecl'] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
        part2 = part2 and len(d['pid']) == 9 and all([x >= '0' and x <= '9' for x in d['pid']])
        if part2:
            part2_valid += 1
print(part1_valid)
print(part2_valid)